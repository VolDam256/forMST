...
var semi_text_new = document.querySelectorAll(".new");

var navigacia = document.querySelector(".navigacia");
...
addblackbox(semi_text_new[0], 0);

function addblackbox(text, nomer) {
  var text_new = text.textContent;
  if (text.textContent.length > 165) {
    var forslice = text.textContent.lastIndexOf(" ");
    text_new = text_new.slice(0, forslice);
  }
  semi_text_new[nomer].textContent = text_new;
  semi_text_new[nomer].appendChild(e);
}
...
function listremove(string, colline, nomer) {
  ...
  string.onclick = function () {
    if (nomer != whataname) {
      /*лИНИЯ*/
      list_content_text[whataname].classList.remove("list_content-main");
      list_content_text[whataname].classList.add("list_content-semi");
      color_line[whataname].style.width = "0";
      /*тЕКСТ И ЛИНИЯ*/
      string.classList.remove("list_content-semi");
      string.classList.add("list_content-main");
      color_line[nomer].style.width = "100%";
      //description_content[nomer].classList.add("description_content_active");

      /*бЛОК*/
      description_item[whataname].classList.remove("description_item-active");
      description_item[nomer].classList.add("description_item-active");
      navigacia.textContent = nomer + 1 + "/6";
      /*кАРТИНКА*/
      picture_img[5 - whataname].classList.remove("picture_img_active");
      picture_img[5 - nomer].classList.add("picture_img_active");

      /* Текст */
      addblackbox(semi_text_new[nomer], nomer);
